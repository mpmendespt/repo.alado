...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if yes:
			...
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Obrigado por escolher a Milhano Build, para salvar as alteracoes e finalizar a instalacao, voce agora precisa forcar o fechemanto do Kodi, por favor, pressione OK para encerrar o Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
...
def Installables():
	...
	APKButton = pyxbmct.Button('[COLOR %s][B] APK [/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
	...
	EmuButton = pyxbmct.Button('[COLOR %s][B]TEST[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
